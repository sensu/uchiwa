...
Sensu.prototype.deleteClient = function(data, callback){
  var obj = data;
  var options = {
    host: this.host,
    ssl: this.ssl,
    port: this.port,
    path: this.path+'/clients/'+obj.path,
    method: 'DELETE',
    timeout: this.timeout,
    headers: this.headers
  };
...
Sensu.prototype.buildClients = function(callback){
  ...
  async.each(this.clients, function(client, next){
    ...
    if(!client.version) client.version = "<= 0.12.6-5";
...
Sensu.prototype.eventData = function(self, event){
  ...
  if(!_.has(event, "id")){
    ...
    eventData.client = {name: fullClient.name, address: fullClient.address, subscription: fullClient.subscription, version: "<= 0.12.6-5"};
