...
Dc.prototype.getStashes = function (callback) {
  ...
  this.sensu.get('stashes', function (err, result) {
    ...
    if (!err) {
      self.sensu.getTimestamp(self.sensu.stashes, 'timestamp', 'last_check', function () {
      });
    }
    if (!err) {
...
Dc.prototype.getClients = function (callback) {
  ...
  this.sensu.get('clients', function (err, result) {
    ...
    if (!err) {
      self.sensu.getTimestamp(self.sensu.clients, 'timestamp', 'last_check', function () {
      });
    }
    callback(err);
  });
...
Dc.prototype.getClient = function (clientName, callback) {
  ...
  this.sensu.getClient(clientName, function (err, result) {
    ...
    if (!err) {
      self.sensu.getTimestamp(client.history, 'last_execution', 'last_check', function () {
      });
    }
    callback(err, client);
  });
