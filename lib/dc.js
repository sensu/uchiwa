...
Dc.prototype.getStashes = function (callback) {
  ...
  this.sensu.get('stashes', function (err, result) {
    ...
    if (!err) {
      self.sensu.getTimestamp(self.sensu.stashes, 'timestamp', 'lastCheck', function () {
      });
    }
    if (!err) {
...
Dc.prototype.getClient = function (clientName, callback) {
  ...
  this.sensu.getClient(clientName, function (err, result) {
    ...
    if (!err) {
      self.sensu.sortHistory(client.history, 'check', 'lastStatus', function () {
      });
    }
    if (!err) {
      self.sensu.getTimestamp(client.history, 'lastExecution', 'lastCheck', function () {
      });
    }
    callback(err, client);
  });
